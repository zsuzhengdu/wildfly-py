...
class ServerApiMixin(object):
    ...
    def servers(self, server_group=None, host=None):
        ...
        for host in hosts:
            address = [{'host': host}]
            servers_config = self.read_children_resources('server-config', address, True)
            ...
            for key, value in servers.items():
                if value['server-state'] != 'STOPPED':
                    ...
                    uptime = self.read_attribute(address=address, name='uptime')
                else:
                    uptime = None
                if server_group is None or servers_config[key]['group'] == server_group:
                    # STARTING, RUNNING, STOPPED or RESTART_REQUIRED
                    servers_merged[key] = {'group': servers_config[key]['group'],
                                           'host': host,
