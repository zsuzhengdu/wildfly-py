...
DEFAULT_DEPLOYMENT_NAME = '{}-{}.{}'.format(DEFAULT_ARTIFACT_ID, DEFAULT_ARTIFACT_VERSION, DEFAULT_ARTIFACT_TYPE)
...
class DeploymentTest(base.BaseTestCase):
  ...
  ## TODO: ADD Test to cover option: host. Currently, host name is docker instance ip in fixture setup.
  ...
  def test_list_deployments(self):
    try:
      ...
      self.assertEqual(self.client.deployment_status(DEFAULT_DEPLOYMENT_NAME), 'RUNNING')
    except Exception as e:
      self.fail(base.UNEXPECTED_EXCEPTION_FORMAT.format(e))
    finally:
      self.client.undeploy(DEFAULT_DEPLOYMENT_NAME)
  ...
  def test_list_deployments_disabled(self):
    try:
      ...
      self.assertEqual(self.client.deployment_status(DEFAULT_DEPLOYMENT_NAME), 'STOPPED')
    except Exception as e:
      self.fail(base.UNEXPECTED_EXCEPTION_FORMAT.format(e))
    finally:
      self.client.undeploy(DEFAULT_DEPLOYMENT_NAME)
  ...
  def test_list_deployments_server_down(self):
    try:
      ...
      self.assertEqual(deployments[DEFAULT_DEPLOYMENT_NAME]['status'], 'STOPPED')
    except Exception as e:
      self.fail(base.UNEXPECTED_EXCEPTION_FORMAT.format(e))
    finally:
      self.client.undeploy(DEFAULT_DEPLOYMENT_NAME)
  ...
  def test_is_deployment_in_repository(self):
    try:
      self.client.deploy(DEFAULT_GROUP_ID, DEFAULT_ARTIFACT_ID, DEFAULT_ARTIFACT_VERSION)
      result = self.client.is_deployment_in_repository(DEFAULT_DEPLOYMENT_NAME)
...
class DeployTest(base.BaseTestCase):
  ...
  def test_deploy_url(self):
    ...
    try:
      ...
      self.assertIn(DEFAULT_DEPLOYMENT_NAME, deployments)
      self.assertTrue(deployments[DEFAULT_DEPLOYMENT_NAME]['enabled'])
      self.assertEqual(deployments[DEFAULT_DEPLOYMENT_NAME]['status'], 'RUNNING')
    except Exception as e:
      self.fail(base.UNEXPECTED_EXCEPTION_FORMAT.format(e))
    finally:
      self.client.undeploy(DEFAULT_DEPLOYMENT_NAME)
